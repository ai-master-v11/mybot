...
# ==================================================================
# 🎙️ 6. ADVANCED HINDI AI VOICE ASSISTANT (AYAN MODE FOR MASUM BHAI)
# ==================================================================
st.markdown("""
    <div class="ayan-box">
        <div class="ayan-title">🎙️ AYAN - YOUR HINDI AI ASSISTANT</div>
        <p style="font-size:13px; color:#8892b0; margin-top:5px;">
            Click below to speak in Hindi or English with AYAN AI.
        </p>
    </div>
""", unsafe_allow_html=True)
...
if (SpeechRecognition) {{
    ...
    recognition.lang = 'hi-IN'; // Multi-language recognition (Hindi/English)

    talkBtn.addEventListener('click', () => {{
        ayanResponse.innerText = "Listening... Sun raha hu Masum Bhai...";
        recognition.start();
    }});
    ...
    recognition.onresult = (event) => {{
        ...
        if (userText.includes("kaise ho") || userText.includes("kaisa hai") || userText.includes("status")) {{
            reply = "Haan Masum Bhai, mai ekdam mast hu! Sabhi system perfectly kaam kar rahe hain.";
        }} else if (userText.includes("market") || userText.includes("trade") || userText.includes("signal")) {{
            reply = "Haan Masum Bhai, {selected_currency} ka market analysis ready hai. Aap signal box check kar sakte ho.";
        }} else if (userText.includes("ayan")) {{
            reply = "Haan Masum Bhai bolo, mai aapki kya madad kar sakta hu?";
        }} else {{
            reply = "Haan Masum Bhai, maine aapki baat sun li hai. Bataiye aur kya karna hai?";
        }}
...
function speakAyan(text) {{
    ...
    utterance.lang = 'hi-IN'; // Hindi Speech Synthesis Output
    utterance.pitch = 1.0;
    utterance.rate = 0.95;
