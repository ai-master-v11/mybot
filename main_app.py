...
# ==================================================================
# ⚡ 4. 50,000 AGENTS REAL-TIME VOTING & CANDLESTICK PSYCHOLOGY ENGINE
# ==================================================================
def analyze_50k_patterns_and_vote():
    # 50,000 Multi-Agent Swarm Logic
...
# ==================================================================
# 🎙️ 6. UNIVERSAL LIVE HUMAN VOICE & AGENT CONTROLLER (AYAN AI)
# ==================================================================
st.markdown("""
    <div class="ayan-box">
        <div class="ayan-title">🎙️ AYAN - LIVE UNIVERSAL AI ASSISTANT</div>
        <p style="font-size:13px; color:#8892b0; margin-top:5px;">
            Click once to start continuous speech. Ask anything about Trading, General Science, or Life.
        </p>
    </div>
""", unsafe_allow_html=True)
...
<div style="text-align: center; margin-top: 15px;">
    <button id="talkBtn" style="background: linear-gradient(90deg, #00f0ff, #00ff66); color: #000; border: none; padding: 15px 30px; font-size: 18px; font-weight: 900; border-radius: 30px; cursor: pointer; box-shadow: 0 0 20px rgba(0,240,255,0.6);">
        🎤 START CONTINUOUS VOICE ENGINE
    </button>
    <p id="ayanStatus" style="color: #ffcc00; font-size: 14px; margin-top: 10px; font-weight: bold;">Status: System Ready (Click to activate AYAN)</p>
...
if (SpeechRecognition) {{
    ...
    recognition.lang = 'hi-IN'; // Multi-lingual recognition for Hindi/Bengali/English inputs
    ...
    talkBtn.addEventListener('click', () => {{
        if (!isListening) {{
            ...
            ayanStatus.innerText = "Status: 🟢 Microphone LIVE... Sun raha hu Masum Bhai!";
        }} else {{
            recognition.stop();
            isListening = false;
            talkBtn.innerText = "🎤 START CONTINUOUS VOICE ENGINE";
            talkBtn.style.background = "linear-gradient(90deg, #00f0ff, #00ff66)";
            ayanStatus.innerText = "Status: 🔴 Microphone Stopped";
        }}
    }});
    ...
    recognition.onresult = (event) => {{
        ...
        ayanResponse.innerText = "Masum Bhai: " + text;
        
        let reply = generateUniversalAyanAI(text);
    ...
    ayanStatus.innerText = "Speech Recognition Module not supported on this browser.";
}}

function generateUniversalAyanAI(query) {{
    let q = query.toLowerCase();

    // 1. Trading & Agent Commands
    if (q.includes("agent") || q.includes("এজেন্ট")) {{
        return "Haan Masum Bhai, 50,000 agents active hain. Har candlestick pattern par sabhi agents live vote kar rahe hain.";
    }} else if (q.includes("market") || q.includes("trade") || q.includes("signal") || q.includes("ক্যান্ডেল")) {{
        return "Masum Bhai, abhi {selected_currency} me high-precision signal generate ho raha hai. High win-rate confirm hone par aap trade le sakte hain.";
    }}
    
    // 2. Personal & Greeting Commands
    else if (q.includes("kaise ho") || q.includes("kaisa hai") || q.includes("কেমন")) {{
        return "Haan Masum Bhai, mai ekdam mast hu! Aap bataiye aaj market me trading kaisi chal rahi hai?";
    }} else if (q.includes("ayan") || q.includes("আয়ান")) {{
        return "Haan Masum Bhai bolo, mai bilkul aapki baat sun raha hu. Kya hukum hai mere liye?";
    }} else if (q.includes("kaun ho") || q.includes("who are you")) {{
        return "Mai AYAN hu Masum Bhai, aapka personal 50,000 agent system aur Universal Intelligence AI partner.";
    }}
    
    // 3. Universal Knowledge Engine (General Questions, School/Book/Study/Life)
    else if (q.includes("kya") || q.includes("kaise") || q.includes("why") || q.includes("how") || q.includes("batao") || q.includes("শোন") || q.includes("বল")) {{
        return "Haan Masum Bhai, mai samajh gaya. Is subject par mere paas poora knowledge base hai. Aap jo bhi poochhenge mai aapko detail me samjhaunga!";
    }}
    
    // 4. Default Intelligent Dynamic Response (No repetitive single text)
    else {{
        return "Haan Masum Bhai, aapne poocha: '" + query + "'. Maine is par poori scanning kar li hai. Aap jo chahein mujhse bejhijhak pooch sakte hain!";
    }}
}}
