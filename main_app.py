...
# ==================================================================
# 🌌 ১. ভিডিওর মতো রিয়েল-টাইম লাইভ ক্যান্ডেল গ্রাফিক্স ডিজাইন (CSS)
# ==================================================================
st.set_page_config(page_title="Finorix AI - Live Candle Display", layout="centered")

st.markdown("""
    <style>
    .reportview-container { background-color: #06090e; }
    .main-card { background: linear-gradient(145deg, #0b111e, #050811); padding: 40px; border-radius: 25px; border: 1px solid #1e293b; text-align: center; box-shadow: 0px 15px 35px rgba(0,0,0,0.8); max-width: 500px; margin: 0 auto; }
    
    /* ক্যান্ডেলের ভেতরের লাইভ টাইমার টেক্সট */
    .candle-timer-inside { font-size: 26px !important; font-weight: 800; color: #ffffff; font-family: 'monospace'; margin-top: 45px; text-shadow: 0px 0px 8px rgba(0,0,0,0.5); }
    
    /* অ্যাডভান্স সিগন্যাল টেক্সট */
    .text-buy { font-size: 40px !important; font-weight: 900; color: #00ffcc; text-shadow: 0px 0px 20px #00ffcc; margin-bottom: 20px; }
    .text-sell { font-size: 40px !important; font-weight: 900; color: #ff3366; text-shadow: 0px 0px 20px #ff3366; margin-bottom: 20px; }
    
    /* ক্যান্ডেল আর্কিটেকচার এবং শ্যাডো (Wick) */
    .candle-container { display: inline-block; position: relative; padding: 10px; }
    .top-wick { width: 4px; background-color: #ffffff; margin: 0 auto; transition: height 0.5s ease; }
    .bottom-wick { width: 4px; background-color: #ffffff; margin: 0 auto; transition: height 0.5s ease; }
    
    /* ভিডিওর মতো লাইভ ক্যান্ডেল বডি এবং ফ্ল্যাকচুয়েশন অ্যানিমেশন */
    .body-green { width: 75px; background: linear-gradient(to bottom, #00ffcc, #00aa88); margin: 0 auto; box-shadow: 0px 0px 35px rgba(0,255,204,0.6); border-radius: 4px; transition: height 0.3s ease; display: flex; flex-direction: column; justify-content: center; align-items: center; }
    .body-red { width: 75px; background: linear-gradient(to bottom, #ff3366, #b3002d); margin: 0 auto; box-shadow: 0px 0px 35px rgba(255,51,102,0.6); border-radius: 4px; transition: height 0.3s ease; display: flex; flex-direction: column; justify-content: center; align-items: center; }
    </style>
    """, unsafe_allow_html=True)

# ==================================================================
# 🧠 ২. সেশন স্টেট মেকানিজম (ভিডিওর মতো ডেটা লক অ্যান্ড রিস্টার্ট)
# ==================================================================
if 'advance_signal' not in st.session_state:
    st.session_state.advance_signal = "GREEN (BUY)"
if 'candle_height' not in st.session_state:
    st.session_state.candle_height = 140
if 'top_wick_height' not in st.session_state:
    st.session_state.top_wick_height = 30
if 'bottom_wick_height' not in st.session_state:
    st.session_state.bottom_wick_height = 30

# ==================================================================
# ⏰ ৩. ওটিসি লাইভ ঘড়ি সিঙ্ক এবং রিয়েল ক্যান্ডেল কাঁপানো (Fluctuation) লজিক
# ==================================================================
# রিয়েল ঘড়ির সেকেন্ড ট্র্যাক করা
live_seconds = time.localtime().tm_sec
remaining_seconds = 60 - live_seconds

# প্রতি ১ মিনিট পর পর (সেকেন্ড যখন ০ হবে) ভিডিওর মতো ক্যান্ডেল কালার অটো-ফ্লিপ বা রিস্টার্ট হবে
if remaining_seconds == 60 or remaining_seconds == 0:
    st.session_state.advance_signal = random.choice(["GREEN (BUY)", "RED (SELL)"])
    st.session_state.candle_height = 140  # ক্যান্ডেল রিসেট

# ভিডিওতে যেভাবে লাইভ মার্কেট চলার সময় ক্যান্ডেলটি অনবরত ছোট-বড় হয়ে কাঁপছিল, সেই লজিক:
# প্রতি সেকেন্ডে ক্যান্ডেলের হাইট এবং উইক সামান্য র্যান্ডমাইজ হবে যাতে লাইভ ভাইব্রেশন বা নড়াচড়া তৈরি হয়
st.session_state.candle_height = random.randint(120, 170)
st.session_state.top_wick_height = random.randint(20, 45)
st.session_state.bottom_wick_height = random.randint(20, 45)

# ==================================================================
# 🖥️ ৪. সম্পূর্ণ ভিডিও লেআউট ডিসপ্লে প্যানেল (সেম টু সেম)
# ==================================================================
st.markdown("<div class='main-card'>", unsafe_allow_html=True)
st.markdown("<h2 style='color: #8f9cae; letter-spacing: 2px; font-size: 22px;'>🔮 1-CANDLE AHEAD LIVE ENGINE</h2>", unsafe_allow_html=True)
st.write("---")

# ভিডিওর মতো ১ ঘর আগের এডভান্স সিগন্যাল টেক্সট
if st.session_state.advance_signal == "GREEN (BUY)":
    st.markdown("<p class='text-buy'>⬆️ NEXT CANDLE: GREEN</p>", unsafe_allow_html=True)
    
    # ভিডিওর মতো লাইভ চলন্ত গ্রিন ক্যান্ডেল আর্কিটেকচার (উইক এবং টাইমারসহ)
    st.markdown(f"""
    <div class='candle-container'>
        <div class='top-wick' style='height: {st.session_state.top_wick_height}px;'></div>
        <div class='body-green' style='height: {st.session_state.candle_height}px;'>
            <div class='candle-timer-inside'>00:{remaining_seconds:02d}</div>
        </div>
        <div class='bottom-wick' style='height: {st.session_state.bottom_wick_height}px;'></div>
    </div>
    """, unsafe_allow_html=True)
else:
    st.markdown("<p class='text-sell'>⬇️ NEXT CANDLE: RED</p>", unsafe_allow_html=True)
    
    # ভিডিওর মতো লাইভ চলন্ত রেড ক্যান্ডেল আর্কিটেকচার (উইক এবং টাইমারসহ)
    st.markdown(f"""
    <div class='candle-container'>
        <div class='top-wick' style='height: {st.session_state.top_wick_height}px;'></div>
        <div class='body-red' style='height: {st.session_state.candle_height}px;'>
            <div class='candle-timer-inside'>00:{remaining_seconds:02d}</div>
        </div>
        <div class='bottom-wick' style='height: {st.session_state.bottom_wick_height}px;'></div>
    </div>
...
st.write("---")
st.markdown("<p style='color: #566573; font-size: 14px;'>STATUS: REAL-TIME OTC CANDLE FLUID SYNC ACTIVE</p>", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True)

# ==================================================================
# 🔄 ৫. ভিডিওর মতো ক্যান্ডেলকে প্রতি সেকেন্ডে লাইভ স্ক্রিনে সচল রাখার লুপ
# ==================================================================
time.sleep(1)
st.rerun()
#
