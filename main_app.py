...
# ==================================================================
# 🎙️ 6. UNIVERSAL LIVE VOICE ASSISTANT (ELEVENLABS HUMAN VOICE CORE)
# ==================================================================
st.markdown("""
    <div class="ayan-box">
        <div class="ayan-title">🎙️ AYAN - SMART HUMAN VOICE ASSISTANT</div>
        <p style="font-size:13px; color:#8892b0; margin-top:5px;">
            অন করুন এবং আপনার পছন্দের ভাষায় যেকোনো কিছু জিজ্ঞেস করুন। (ElevenLabs Human Bengali Voice Active)
        </p>
    </div>
""", unsafe_allow_html=True)
...
<div style="text-align: center; margin-top: 15px;">
    <button id="talkBtn" style="background: linear-gradient(90deg, #00f0ff, #00ff66); color: #000; border: none; padding: 15px 30px; font-size: 18px; font-weight: 900; border-radius: 30px; cursor: pointer; box-shadow: 0 0 20px rgba(0,240,255,0.6);">
        🎤 START HUMAN VOICE ASSISTANT
    </button>
    <p id="ayanStatus" style="color: #ffcc00; font-size: 14px; margin-top: 10px; font-weight: bold;">Status: ElevenLabs Core Ready</p>
    <p id="ayanResponse" style="color: #00ff66; font-family: monospace; font-size: 16px; margin-top: 10px; font-weight: bold;"></p>
    <audio id="elevenAudio" style="display:none;"></audio>
</div>
...
const elevenAudio = document.getElementById('elevenAudio');

const API_KEY = "{elevenlabs_api_key}";
const VOICE_ID = "21m00Tcm4TlvDq8ikWAM"; // Aria Human Voice ID
...
if (SpeechRecognition) {{
    ...
    talkBtn.addEventListener('click', () => {{
        if (!isListening) {{
            ...
            ayanStatus.innerText = "Status: 🟢 Listening LIVE... বলুন মাসুম ভাই!";
        }} else {{
            recognition.stop();
            isListening = false;
            talkBtn.innerText = "🎤 START HUMAN VOICE ASSISTANT";
    ...
    recognition.onresult = (event) => {{
        ...
        let reply = generateSmartAnswer(text);
    ...
    ayanStatus.innerText = "Speech Recognition Engine not supported in this browser.";
}}
...
function generateSmartAnswer(query) {{
    ...
    if (q.includes("agent") || q.includes("এজেন্ট")) {{
        return "হ্যাঁ মাসুম ভাই, ৫০,০০০ এজেন্ট পুরোপুরি সক্রিয় আছে এবং ব্যাকএন্ডে মার্কেট রিড করছে।";
    }} else if (q.includes("market") || q.includes("trade") || q.includes("signal") || q.includes("মার্কেট")) {{
        return "মাসুম ভাই, ৫০,০০০ এজেন্ট এনালাইসিস শেষ করে সিগন্যাল স্ক্রিনে শো করছে।";
    }} else if (q.includes("কেমন") || q.includes("আছো")) {{
        return "আমি খুব ভালো আছি মাসুম ভাই! আপনার আজকের ট্রেডিং কেমন চলছে?";
    }} else if (q.includes("ayan") || q.includes("আয়ান")) {{
        return "হ্যাঁ মাসুম ভাই বলুন, আয়ান আপনার সেবায় উপস্থিত।";
    }} else {{
        return "হ্যাঁ মাসুম ভাই, আমি আপনার কথা শুনতে পেয়েছি। আপনি নিশ্চিন্তে কাজ চালিয়ে যেতে পারেন!";
    }}
}}

async function speakElevenLabs(text) {{
    if (!API_KEY) {{
        ayanStatus.innerText = "Status: ⚠️ ElevenLabs API Key পাওয়া যায়নি!";
        return;
    }}
    
    ayanStatus.innerText = "Status: 🔊 ElevenLabs দিয়ে মানুষের মতো কথা বলছে...";
    ...
    try {{
        ...
        ayanStatus.innerText = "Status: ⚠️ অডিও জেনারেট করতে সমস্যা হয়েছে";
    }}
}}
</script>
"""
