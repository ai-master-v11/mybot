...
# ==================================================================
# 🎙️ 6. ALWAYS-ON CONTINUOUS AI VOICE ASSISTANT (AYAN SMART ENGINE)
# ==================================================================
st.markdown("""
    <div class="ayan-box">
        <div class="ayan-title">🎙️ AYAN - CONTINUOUS LIVE HUMAN VOICE ENGINE</div>
        <p style="font-size:13px; color:#8892b0; margin-top:5px;">
            Click once to turn ON microphone. Speak continuously without clicking again.
        </p>
    </div>
""", unsafe_allow_html=True)
...
<div style="text-align: center; margin-top: 15px;">
    <button id="talkBtn" style="background: linear-gradient(90deg, #00f0ff, #00ff66); color: #000; border: none; padding: 15px 30px; font-size: 18px; font-weight: 900; border-radius: 30px; cursor: pointer; box-shadow: 0 0 20px rgba(0,240,255,0.6);">
        🎤 START CONTINUOUS VOICE (AYAN)
    </button>
    <p id="ayanStatus" style="color: #ffcc00; font-size: 14px; margin-top: 10px; font-weight: bold;">Status: Idle (Click button to start)</p>
...
if (SpeechRecognition) {{
    ...
    recognition.lang = 'hi-IN'; // Multi-language understanding (Hindi/Bengali/English mixed)
    ...
    talkBtn.addEventListener('click', () => {{
        if (!isListening) {{
            recognition.start();
            isListening = true;
            talkBtn.innerText = "🛑 STOP VOICE LISTENING";
            talkBtn.style.background = "linear-gradient(90deg, #ff0055, #ffcc00)";
            ayanStatus.innerText = "Status: 🟢 Listening continuously... Speak freely Masum Bhai!";
        }} else {{
            recognition.stop();
            isListening = false;
            talkBtn.innerText = "🎤 START CONTINUOUS VOICE (AYAN)";
            talkBtn.style.background = "linear-gradient(90deg, #00f0ff, #00ff66)";
            ayanStatus.innerText = "Status: 🔴 Voice Stopped";
        }}
    }});

    recognition.onresult = (event) => {{
        const lastIndex = event.results.length - 1;
        const userText = event.results[lastIndex][0].transcript.trim().toLowerCase();
        ayanResponse.innerText = "Masum Bhai: " + userText;
        
        let reply = generateSmartAyanReply(userText);
    ...
    recognition.onend = () => {{
        // Auto-restart if user did not manually stop it
        if (isListening) {{
            try {{
                recognition.start();
            }} catch(e) {{}}
    ...
    ayanStatus.innerText = "Browser does not support continuous Speech Recognition.";
}}

function generateSmartAyanReply(input) {{
    // Intelligent response generator for all subjects
    if (input.includes("kaise ho") || input.includes("kaisa hai") || input.includes("kemon acho")) {{
        return "Haan Masum Bhai, mai bilkul badiya hu! Aap bataiye aaj ka din kaisa chal raha hai?";
    }} else if (input.includes("market") || input.includes("trade") || input.includes("pair") || input.includes("currency")) {{
        return "Masum Bhai, abhi {selected_currency} market me pattern analysis chal raha hai. Aap high win-rate ke liye screen ka signal box dekh sakte ho!";
    }} else if (input.includes("girlfriend") || input.includes("prem") || input.includes("pyaar")) {{
        return "Ha ha, Masum Bhai! Meri koi girlfriend nahi hai, mai toh bas 24 ghante aapke saath trading aur baatein karne me busy rehta hu!";
    }} else if (input.includes("ayan") || input.includes("aayan")) {{
        return "Haan Masum Bhai, mai sun raha hu! Bataiye kya jaan na chahte hain aap?";
    }} else if (input.includes("kaam") || input.includes("help") || input.includes("batao")) {{
        return "Masum Bhai, aap mujhse trading, market psychology, ya duniya ke kisi bhi topic par baat kar sakte hain. Mai sab jaanta hu!";
    }} else {{
        return "Haan Masum Bhai, maine aapki baat achhe se samajh li. Aap jab chahe mujhse koi bhi sawal pooch sakte hain, mai aapko poori jankari dunga!";
    }}
}}

function speakAyan(text) {{
    const synth = window.speechSynthesis;
    // Stop any existing speech before talking again
    if (synth.speaking) {{
        synth.cancel();
    }}
