...
# ==================================================================
# 🌌 ১. ভিডিওর মতো লাক্সারি ব্ল্যাক থিম ও রিয়েল চার্ট ইন্টারফেস (CSS)
# ==================================================================
st.set_page_config(page_title="AI MASTER V14 - Live Chart Engine", layout="wide")

st.markdown("""
    <style>
    .stApp { background-color: #06090e; color: #ffffff; }
    .glow-title { font-size: 28px !important; font-weight: 800; color: #00FFCC; text-shadow: 0px 0px 15px rgba(0,255,204,0.6); text-align: center; }
    .signal-card { background: #0c1424; padding: 15px; border-radius: 12px; border: 1px solid #1e293b; text-align: center; }
    .text-buy { font-size: 30px !important; font-weight: 900; color: #00ffcc; text-shadow: 0px 0px 15px #00ffcc; }
    .text-sell { font-size: 30px !important; font-weight: 900; color: #ff3366; text-shadow: 0px 0px 15px #ff3366; }
    .timer-style { font-size: 24px !important; font-weight: bold; color: #ff9900; font-family: monospace; }
    </style>
    """, unsafe_allow_html=True)

# ==================================================================
# 🎛️ ২. বাম পাশের সাইডবারে স্ক্রিনশটের হুবহু "Live Quantum Feeds" প্যানেল
# ==================================================================
with st.sidebar:
    st.markdown("### 🎛️ Live Quantum Feeds")
    st.write("---")
    market_vector = st.selectbox("Market Dominant Vector Direction", ["BUY", "SELL"], index=0)
    photon_velocity = st.slider("Photon Price Velocity", 100000, 500000, 280333)
    inst_flux = st.slider("Institutional Volumetric Flux", 0.00, 5.00, 4.02)
    chaos_score = st.slider("Market Randomness Chaos Score", 0.00, 1.00, 0.04)
    predicted_drawdown = st.slider("Future Predicted Drawdown (%)", 0.00, 10.00, 1.80)
    extrapolation_curve = st.slider("Future Candle Extrapolation Curve", 0.00, 1.00, 0.95)
    hyper_alignment = st.checkbox("Hyper Multi-Timeframe Alignment", value=True)
    same_color_candles = st.slider("Continuous Same-Color Candles", 1, 15, 3)
    current_otc_phase = st.selectbox("Current OTC Phase", ["UPWARD", "DOWNWARD"], index=0)

# ==================================================================
# 🧠 ৩. রিয়েল-টাইম লাইভ ক্যান্ডেলস্টিক চার্ট ডেটা জেনারেটর (Chart History)
# ==================================================================
# চার্টের লাইভ হিস্ট্রি বা ক্যান্ডেলের ডাটা সেশন স্টেটে ধরে রাখা
if 'chart_data' not in st.session_state:
    # শুরুতে চার্টটি খালি না রেখে ২০টি তৈরি করা ক্যান্ডেল দিয়ে ব্যাকগ্রাউন্ডে শুরু করা হলো
    base_price = 1.2500
    dates = pd.date_range(end=pd.Timestamp.now(), periods=20, freq='min')
    data = []
    for d in dates:
        open_p = base_price + random.uniform(-0.0010, 0.0010)
        ...
        data.append([d, open_p, high_p, low_p, close_p])
        base_price = close_p
    
    st.session_state.chart_data = pd.DataFrame(data, columns=['Time', 'Open', 'High', 'Low', 'Close'])
    st.session_state.last_time = dates[-1]

# ওটিসি ঘড়ির লাইভ সেকেন্ড রিড করা
live_seconds = time.localtime().tm_sec
remaining_seconds = 60 - live_seconds

# প্রতি ১ মিনিট (৬০ সেকেন্ড) পর পর চার্টে একটি নতুন ক্যান্ডেলস্টিক যোগ হবে (Quotex এর মতো)
if remaining_seconds == 60 or remaining_seconds == 0:
    ...
    open_p = last_row['Close']
    # সাইডবারের Vector Direction BUY হলে সবুজ ক্যান্ডেল এবং SELL হলে লাল ক্যান্ডেল তৈরি হবে
    if market_vector == "BUY":
        close_p = open_p + random.uniform(0.0002, 0.0008)
    else:
        close_p = open_p - random.uniform(0.0002, 0.0008)
    ...
    new_candle = pd.DataFrame([[new_time, open_p, high_p, low_p, close_p]], columns=['Time', 'Open', 'High', 'Low', 'Close'])
    st.session_state.chart_data = pd.concat([st.session_state.chart_data, new_candle], ignore_index=True)
    
    # চার্ট যাতে অসীম বড় না হয়ে যায় তাই শেষ ৩০টি ক্যান্ডেল স্ক্রিনে রাখা হবে
    if len(st.session_state.chart_data) > 30:
        st.session_state.chart_data = st.session_state.chart_data.iloc[1:].reset_index(drop=True)

# লাইভ ক্যান্ডেলের ওঠানামা (Fluctuation) - চার্টের একদম শেষ ক্যান্ডেলটি প্রতি সেকেন্ডে কাঁপবে
last_idx = len(st.session_state.chart_data) - 1
st.session_state.chart_data.at[last_idx, 'Close'] += random.uniform(-0.0001, 0.0001)

# ==================================================================
# 🖥️ ৪. ডান পাশে মূল স্ক্রিনে কারেন্সি ড্রপডাউন এবং রিয়েল ক্যান্ডেলস্টিক চার্ট
# ==================================================================
st.markdown("<p class='glow-title'>⚡ FINORIX AI MASTER V14 - LIVE MATRIX</p>", unsafe_allow_html=True)
st.write("---")

col_control, col_timer = st.columns([2, 1])
with col_control:
    selected_asset = st.selectbox("CHOOSE LIVE PAIR (কারেন্সি পেয়ার সিলেক্ট করুন):", ["USD/PKR OTC", "EUR/USD OTC", "USD/BDT OTC", "GBP/USD OTC"])
with col_timer:
    st.markdown("<div class='signal-card'>", unsafe_allow_html=True)
    st.markdown(f"**CANDLE TIMER:** <span class='timer-style'>00:{remaining_seconds:02d}</span>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

st.write("---")

col_chart_view, col_signal_view = st.columns([3, 1])

with col_chart_view:
    # ব্রোকারের মতো রিয়েল ক্যান্ডেলস্টিক চার্ট তৈরি (Plotly Graph)
...
with col_signal_view:
    st.markdown("<div class='signal-card' style='height: 100%;'>", unsafe_allow_html=True)
    st.markdown("### 🔮 ADVANCE SIGNAL")
    st.write("---")
    
    # ১ ক্যান্ডেল আগের অগ্রিম প্রেডিকশন মেসেজ
    if market_vector == "BUY":
        st.markdown("<p class='text-buy'>⬆️ NEXT<br>GREEN<br>(CALL)</p>", unsafe_allow_html=True)
    else:
        st.markdown("<p class='text-sell'>⬇️ NEXT<br>RED<br>(PUT)</p>", unsafe_allow_html=True)
        
    st.write("---")
    st.caption(f"Asset: {selected_asset}")
    st.caption("Mode: 1-Candle Ahead Sync")
    st.markdown("</div>", unsafe_allow_html=True)

# ==================================================================
# 🔄 ৫. চার্টকে প্রতি সেকেন্ডে ব্রোকারের মতো লাইভ সচল রাখার লুপ
# ==================================================================
time.sleep(1)
st.rerun()
